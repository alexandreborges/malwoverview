...
def overextract(fname):
    ...
    with open(fname + ".overlay", "wb") as t:
        t.write(r[offset:])
    mag = ftype(fname)
    print(Fore.RED + "\nOverlay extracted: %s.overlay"  % fname)
    print(Fore.YELLOW + "Overlay type:      %s\n"  % mag)
...
if __name__ == "__main__":
    ...
    parser.add_option('-f', dest='fpname',type='string',default = '', help='specify a full path to a file.')
    ...
    parser.add_option('-s', dest='showvt', type='int',default = 0, help='(optional) show antivirus reports from the main players (it is used with -f option).')
